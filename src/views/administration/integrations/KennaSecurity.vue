<template>
  <b-card no-body :header="header">
    <b-card-body>
      <c-switch v-bind="labelIcon" id="enabled" color="primary" v-model="enabled" label />{{$t('admin.integration_kenna_enable')}}
    ...
    methods: {
      saveChanges: function() {
        this.updateConfigProperties([
          {groupName: 'integrations', propertyName: 'kenna.enabled', propertyValue: this.enabled},
          {groupName: 'integrations', propertyName: 'kenna.sync.cadence', propertyValue: this.cadence},
          {groupName: 'integrations', propertyName: 'kenna.connector.id', propertyValue: this.connectorId},
          {groupName: 'integrations', propertyName: 'kenna.token', propertyValue: this.token}
        ]);
      }
    },
    created () {
        ...
        for (let i=0; i<configItems.length; i++) {
          ...
          switch (item.propertyName) {
            ...
            case "kenna.connector.id":
              this.connectorId = item.propertyValue; break;
            case "kenna.token":
